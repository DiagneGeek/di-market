<script lang="ts">
    let {children, label, variant = "primary", size = "lg", "class": _class, ...props} : {
        children?: any,
        label?:any,
        variant?: string,
        class?: string,
...
{:else}
<button
  class="text-[14px] {padding} rounded-2xl font-semibold {type} {_class} disabled:opacity-60 disabled:cursor-not-allowed"
  {...props}
 >
        {label}
 </button>
